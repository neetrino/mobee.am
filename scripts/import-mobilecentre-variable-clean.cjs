...
const {
  ...
  extractMediaUrl,
} = require("./lib/mobilecentre-variant-media.cjs");
...
const JSON_FILE = path.join(ROOT, "mobilecentre_apple_variable_products.json");
const CACHE_FILE = path.join(ROOT, ".mobilecentre-image-cache.json");
...
function buildDescriptionHtml(raw) {
  ...
  const specRows = rows.filter((r) => r.type !== "status");
  let html = "";
