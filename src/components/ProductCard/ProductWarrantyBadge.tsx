...
export function ProductWarrantyBadge({
  ...
  return (
    <div
      className={`pointer-events-none inline-flex flex-col overflow-hidden rounded-xl bg-[#3f5466] text-center not-italic ${className}`}
    ...
    >
      <div className={`flex items-end justify-center gap-0.5 ${styles.padding}`}>
        <span className={`font-bold uppercase text-[#ffca03] ${styles.number}`}>{years}</span>
        <span className="text-[0.875rem] font-normal uppercase leading-[0.9375rem] text-white">
          {t('products.warranty.years_suffix')}
        </span>
      </div>
      <div className="bg-[#ffca03] px-2 py-0.5">
        <span className={`block font-bold uppercase text-[#3f5466] ${styles.label}`}>
