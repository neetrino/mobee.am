...
function StarRow({ averageRating }: { averageRating: number }) {
  const filled = Math.min(5, Math.max(0, Math.round(averageRating)));
  return (
    <div className="flex items-center gap-0.5" aria-label={`Rating ${averageRating} of 5`}>
      {[1, 2, 3, 4, 5].map((star) => (
        <svg
          key={star}
          className={`h-5 w-5 ${star <= filled ? 'text-amber-400' : 'text-gray-300'}`}
          fill="currentColor"
          viewBox="0 0 20 20"
        >
    ...
    <div className="flex flex-col">
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-3">
        ...
        <button
          ...
          className="shrink-0 rounded-sm outline-none ring-offset-2 focus-visible:ring-2 focus-visible:ring-admin"
        >
          <StarRow averageRating={averageRating} />
          {reviewsCount > 0 && (
