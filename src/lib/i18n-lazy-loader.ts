 ...
 * `common`, `home`, `product`, and `products` must be available before lazy loads
 * complete to avoid hydration mismatches on PDP and /shop (filters/sort UI).
 */
...
export async function preloadStorefrontNamespaces(lang: LanguageCode): Promise<void> {
  await preloadNamespaces(lang, ['common', 'home', 'product', 'products']);
  if (lang !== 'en') {
    await preloadNamespaces('en', ['common', 'home', 'product', 'products']);
  }
}
